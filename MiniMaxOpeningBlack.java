/**
 * MiniMax for opening phase played by black
 */
...
public class MiniMaxOpeningBlack {
    public static BoardOut MiniMax(PosList board, int depth, boolean isMin) {
        ...
        if (isMin) {
            ...
            moves = Generate.generateMovesOpening(board);
        }
        else {
            out.val = Integer.MAX_VALUE;
            moves = Generate.generateMovesOpeningForBlack(board);
        }
...
//        String in = "xWxxxWxxBxxxxxBxxx";
        ...
        for (char c : startingBoard.toCharArray())
            ip.add(c);
        BoardOut op = MiniMax(new PosList(ip).swap(), depth, true);

        String result = "Board Position: "+op.list.swap().toString()+"\nPositions evaluated by static estimation: "+op.count
                +"\nMINIMAX estimate: "+op.val+"\ndepth: "+depth;
        BufferedWriter writer = new BufferedWriter(new FileWriter(args[2]));
        writer.write(result);
        writer.close();

//        System.out.println("Minimax val = "+op.val+" Minimax count = "+op.count+" Minimax position = "+op.list.swap().toString());
    }
}
